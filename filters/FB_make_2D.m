...
case 3 %%% similar to Laptev&Lindberg ICPR04
  ...
  r = 9;  sigs = [.5 1 1.5 3]; % sigs = [1,1.5,2];
  ...
  for s=1:length(sigs)
    for i=1:nderivs
      dG = filter_DOOG_2D( r, [sigs(s) sigs(s)], derivs(i,:), 0 );
...
case 6  %%% symmetric DOOG filters
  FB = FB_make_DOOG_symmetric( 4, 2, [.5 1] );
...
function FB = FB_make_DOOG_symmetric( r, num_ori, sigs )  
cnt=1;
for s=1:length(sigs)
  Fodd  = -filter_DOOG_2D( r, [sigs(s) sigs(s)], [0,1], 0 );    
  Feven = filter_DOOG_2D( r, [sigs(s) sigs(s)], [0,2], 0 );    
