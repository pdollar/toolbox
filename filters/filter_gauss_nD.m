...
% point [x,y,z] refers to the x-th column and y-th row, at the t-th frame.
% So for example mu should have the format [col,row,t].
%
% If mu==[], it is calculated to be the center of the image.  C can be a
% full nxn covariance matrix, or an nx1 vector of variance.  In the latter
% case C is calculated as C=diag(C).  If C=[]; then C=(dims/6).^2, ie it is
% transformed into a vector of variances such that along each dimension the
% variance is equal to (siz/6)^2.  
%
% USAGE
%
% INPUTS
