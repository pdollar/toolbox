...
%
% Reconstructs the orginal image from filter outputs. Does this
% independenly for each pixel, and then just combines the pixel info.  Note
% that each patch is 0 mean, since no mean information is captured by the
% filter outputs.  Alter flag below to either attempt to reconstruct the
% entire image of just a patch (interactively specified).  Other flags can
% also be changed, see file.
%
% USAGE
%
% INPUTS
