...
% gaussian window centered at a different location.  The locations are
% evenly spaced throughout the array of size siz.  For example, if M=2,
% then along each dimension d the location of each mask is either 1/4 or
% 3/4 of siz(d) and likewise if M=3 that mask is at 1/6,3/6, or 5/6 of
...
% adjacent masks multiplied by wwidth.  Reducing the widths of the
% gaussians causes there to be less overlap between masks, but if the width
% is reduced too far certain pixels in between masks receive very little
...
% be tiny).  The default value of windowidth is .6, which minimizes overlap
% while still providing good overall coverage. Values lower tend to produce
% noticeable oscillations in coverage.
%
% offset (in [-1,1]) controls the spacing of the locations.  Essentially, a
% positive offset moves the locations away from the center of the array and
% a negative offset moves the windows away from the center.  Using a
% positive offset gives better coverage to areas near the borders.
%
% USAGE
%  [masks,keeplocs] = mask_gaussians( siz, M, wwidth, offset, show )
% 
% INPUTS
%  siz         - dimensions of each mask
%  M           - # mask locations along each dim [either scalar or vector]
%  wwidth      - see above - default: .6
%  offset      - see above - default: .1
...
function [masks,keeplocs] = mask_gaussians( siz, M, wwidth, offset, show )
  nd = length(siz);
  if( nargin<3 || isempty(wwidth)); wwidth = .6; end;    
