...
public:
	...
	void			getDims( int& rows, int& cols )	const;
	bool			resizeTo( const int rows_new, const int cols_new );
	...
	// "*" - ONLY multiplcation of two matricies is NOT pointwise - it is standard multiplication
	...
	#undef DOP
	Matrix operator& (const Matrix &b ) const; // pointwise multiplication
...
template<class T>				Matrix<T>::Matrix(const Matrix& x)
{
	...
	_mrows = 0;
	_ncols = 0;
	SetDimension(x._mrows, x._ncols);
	for (int i = 0; i < size(); i++)
		(*this)(i) = x(i);
}
...
template<class T> bool			Matrix<T>::setDims(const int rows, const int cols)
{
	...
	try {
		...
		cout << "Matrix::SetDimension(..)  OUT OF MEMORY" << endl;
		return false;
	}
...
{
	if( !SetDimension(rows,cols) ) 
		return false;
	setVal( value );
	return true;
}

template<class T> void			Matrix<T>::getDims(int& rows, int& cols) const
{  
	rows = _mrows;
	cols = _ncols;
}
...
{
	if (row>=0 && row<_mrows && col>=0 && col<_ncols)
		return true;
	else
		return false;
}
...
template<class T> bool			Matrix<T>::resizeTo(const int rows_new, const int cols_new)
{
	if( rows_new==rows() && cols_new==cols() )
		return true;

	int lsize = (int)rows_new*cols_new;
	T *data_new, **pdata_new;
	try{
		data_new = new T[lsize];
		pdata_new = new T*[rows_new];
	} catch( bad_alloc& ) {
	...
	for (int j=0; j<rows_new; j++)
		pdata_new[j] = &(data_new[cols_new*j]);
	for (int j=0; j<min((int)rows(),rows_new); j++)
		for (int i=0; i<min((int)cols(),cols_new); i++)
			pdata_new[j][i] = (*this)(j, i);
	Free();
	_mrows = rows_new;
	_ncols = cols_new;
	_data = data_new;
	_dataindex = pdata_new;
	return true;
}
...
{
	Matrix temp;
	temp.SetDimension(_ncols, _mrows);
...
{
	Mdest.SetDimension(Msrc.rows(), Msrc.cols());
...
///////////////////////////////////////////////////////////////////////////////
// Pointwise (Matrix OP Matrix) operations 
#define DOP(OP) \ 
...
#undef DOP
// Pointwise (Matrix<T> OP T) and (T OP Matrix<T>) operations 
#define DOP(OP) \
