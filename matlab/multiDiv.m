function C = multiDiv( A, B, type, method )
...
%
% USAGE
%  C = multiDiv( A, B, type, method )
%
% INPUTS
%  A         - [ma x na x oa] matrix
%  B         - [mb x nb x ob] matrix
%  type      - division type (see above)
%  method    - [1] (svd based) or 2 (QR-based :faster if the A matrices are
%              full rank
%
% OUTPUTS
...
%  tic, C0=multiDiv(A,B,1,2); toc
%  tic, C1=multiDiv(A,B,1); toc
%  tic, C2=C1; for i=1:n, C2(:,:,i)=A(:,:,i)\B; end; toc
%  tic, C3=C1; for i=1:n, C3(:,:,i)=inv(A(:,:,i)); end; toc
%  [ sum(abs(C0(:)-C1(:))), sum(abs(C0(:)-C2(:))), sum(abs(C0(:)-C3(:))) ]
%
%  EXAMPLE [case 2]
%  n=10000; A=randn(3,3,n); B=randn(3,10,n);
%  tic, C0=multiDiv(A,B,2,2); toc
%  tic, C1=multiDiv(A,B,2,1); toc
%  tic, C2=C1; for i=1:n, C2(:,:,i)=A(:,:,i)\B(:,:,i); end; toc
%  [ sum(abs(C0(:)-C1(:))), sum(abs(C0(:)-C2(:))) ]
%  
%  EXAMPLE [case 3]
%  n=10000; A=randn(2,2,n); B=randn(2,n);
%  tic, C0=multiDiv(A,B,3,2); toc
%  tic, C1=multiDiv(A,B,3,1); toc
%  tic, C2=C1; for i=1:n, C2(:,i)=A(:,:,i)\B(:,i); end; toc
%  [ sum(abs(C0(:)-C1(:))), sum(abs(C0(:)-C2(:))) ]
%
